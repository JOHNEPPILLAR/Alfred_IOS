...
class BedHeaterViewController: UIViewController, URLSessionDelegate {
    ...
    @objc func saveSettingsAction(sender: UIBarButtonItem) {
        ...
        // Call API
        let APIbody: Data = try! JSONSerialization.data(withJSONObject: self.bedData[0].dictionaryRepresentation(), options: [])
        let request = putAPIHeaderData(url: "settings/savebed", body: APIbody, useScheduler: true)
        let session = URLSession(configuration: URLSessionConfiguration.default, delegate: self, delegateQueue:OperationQueue.main)
        let task = session.dataTask(with: request, completionHandler: { (data: Data?, response: URLResponse?, error: Error?) -> Void in
            if checkAPIData(apiData: data, response: response, error: error) {
