...
public final class SensorsData: NSCoding {
  ...
  public required init(json: JSON) {
    ...
    if let items = json[SerializationKeys.rows].array { rows = items.map { SensorsRows(json: $0) } }
  }
  ...
  required public init(coder aDecoder: NSCoder) {
    ...
    self.rows = aDecoder.decodeObject(forKey: SerializationKeys.rows) as? [SensorsRows]
  }
